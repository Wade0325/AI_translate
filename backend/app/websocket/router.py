...
# 添加 persistent_uploads 目錄
PERSISTENT_UPLOADS_DIR = Path(
...
def start_celery_task_sync(payload_str: str, file_uid: str) -> None:
    ...
    # 先在 temp_uploads 找，找不到再到 persistent_uploads 找
    temp_file_path = TEMP_UPLOADS_DIR / request_data.filename
    persistent_file_path = PERSISTENT_UPLOADS_DIR / request_data.filename
...
@router.websocket("/{file_uid}")
async def websocket_endpoint(
    websocket: WebSocket,
    file_uid: str
):
    """
    為每個轉錄任務建立一個獨立的 WebSocket 連線。
    """
